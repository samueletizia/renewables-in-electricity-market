...
FOUR_TWO= Model(Gurobi.Optimizer)


@variable(FOUR_TWO, power_DA[1:T]>=0)

@variable(FOUR_TWO, delta[1:T,1:O])          
@variable(FOUR_TWO, delta_more[1:T,1:O]>=0)  
@variable(FOUR_TWO,delta_less[1:T,1:O]>=0)
@variable(FOUR_TWO, epsi)
@variable(FOUR_TWO, mi[1:O]>=0)  

@objective(FOUR_TWO, Max, sum(prob[o]*(scenarios_200[t,DA_price,o]*power_DA[t] 
...
) for t=1:T, o=1:O) + beta[B]*(epsi - 1/(1-alpha)*sum(prob[o]*mi[o] for o=1:O))
)

# if delta>0 and sys=1 is 0.9*DA*delta
# if delta>0 and sys=0 is DA*delta

# if delta<0 and sys=1 is DA*delta
# if delta<0 and sys=0 is 1.2*DA*delta

@constraint(FOUR_TWO,[t=1:T], power_DA[t]<=wind_farm_CAP)
@constraint(FOUR_TWO, [t=1:T,o=1:O], delta[t,o] == scenarios_200[t,DA_wind,o] - power_DA[t])
@constraint(FOUR_TWO, [t=1:T,o=1:O], delta[t,o] == delta_more[t,o] - delta_less[t,o])
@constraint(FOUR_TWO,[o=1:O],-sum(scenarios_200[t,DA_price,o]*power_DA[t] 
